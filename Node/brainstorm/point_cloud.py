...
class PointCloud:
    ...
    def neighbors(self, point, radius: float, min_radius: float = None, include: bool = True) -> np.ndarray:
        """
        :param point:
        :param radius:
        :param include:
        ...
        if min_radius is not None:
            for i, neigh in enumerate(neighbors_array[::-1]):
                if dist(point, neigh) < min_radius:
                    neighbors_array = neighbors_array[len(neighbors_array)-i:]
                    break
        """
    ...
    def divide_areas(
        ...
        while remaining_areas:  # and i < limit:
            ...
            neighbors: List[Tuple[float, float, float]] = [tuple(v_n) for v_n in self.neighbors(
        ...
        # all_points = sum([len(area.points)-len(area.path) for area in areas])
        # for i, area in enumerate(areas):
        #     logger.warning(f"percentage of area {i}: {len(area.points)/all_points}")
...
def test():
    ...
    colors: Union[None, List[Tuple[float, float, float]]] = [
        (244 / 255, 0 / 255, 3 / 255),
        (246 / 255, 129 / 255, 7 / 255),
        (32 / 255, 3 / 255, 255 / 255),
        (79 / 255, 255 / 255, 0 / 255),
    ...
    # robots_helper = [(0, 0, 0) for _ in range(8)]
    ...
    """
    tests = [
        ((5, 5), [(randrange(start=-2, stop=2), randrange(start=-2, stop=2), 0) for _ in range(n_robots)]),
        ((10, 10), [(randrange(start=-4, stop=4), randrange(start=-4, stop=4), 0) for _ in range(n_robots)]),
        ((20, 20), [(randrange(start=-9, stop=9), randrange(start=-9, stop=9), 0) for _ in range(n_robots)]),
        ((30, 30), [(randrange(start=-14, stop=14), randrange(start=-14, stop=14), 0) for _ in range(n_robots)]),
        ((40, 40), [(randrange(start=-18, stop=18), randrange(start=-18, stop=18), 0) for _ in range(n_robots)]),
        ((50, 50), [(randrange(start=-24, stop=24), randrange(start=-24, stop=24), 0) for _ in range(n_robots)]),
        ((75, 75), [(randrange(start=-30, stop=30), randrange(start=-30, stop=30), 0) for _ in range(n_robots)]),
        ((100, 100), [(randrange(start=-45, stop=45), randrange(start=-45, stop=45), 0) for _ in range(n_robots)]),
        ((200, 200), [(randrange(start=-95, stop=95), randrange(start=-95, stop=95), 0) for _ in range(n_robots)]),
        ((400, 400), [(randrange(start=-195, stop=195), randrange(start=-195, stop=195), 0) for _ in range(n_robots)]),
        ((600, 600), [(randrange(start=-295, stop=295), randrange(start=-295, stop=295), 0) for _ in range(n_robots)]),
    ]
    """

    tests = [
        ((5, 5), [(0, 0, 0) for _ in range(n_robots)]),
        ((10, 10), [(0, 0, 0) for _ in range(n_robots)]),
        ((20, 20), [(0, 0, 0) for _ in range(n_robots)]),
        ((30, 30), [(0, 0, 0) for _ in range(n_robots)]),
        ((40, 40), [(0, 0, 0) for _ in range(n_robots)]),
        ((50, 50), [(0, 0, 0) for _ in range(n_robots)]),
        ((75, 75), [(0, 0, 0) for _ in range(n_robots)]),
        ((100, 100), [(0, 0, 0) for _ in range(n_robots)]),
        ((200, 200), [(0, 0, 0) for _ in range(n_robots)]),
        ((400, 400), [(0, 0, 0) for _ in range(n_robots)]),
        ((600, 600), [(0, 0, 0) for _ in range(n_robots)]),
    ]
    """
    tests = [
    ...
    for dimensions, starting_coordinates in tests:
        np.random.seed(100)
        seed(100)
        ...
        # p.pc.points = p.down_sample().points
        # p.refresh()

        start_time = perf_counter()
        # pr.enable()
        pcs = p.divide_areas(starting_points=starting_coordinates, max_neighbor_distance=robot_width)
        # pr.disable()
        logger.warning(f"dimensions: {dimensions} -> "
...
if __name__ == '__main__':
    ...
    for measures in tests:
        for i, v in enumerate(measures[:-1]):
            a = v
            b = measures[i+1]
            percentage_increase = (b - a) / a * 100
