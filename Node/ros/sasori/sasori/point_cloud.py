...
from .lib.point_cloud2 import read_points, create_cloud
...
class PointCloud:
    ...
    def serialize(self, pc_id: str = "pc") -> sensor_msgs.PointCloud2:
        
        if not len(self.pc.points):
            return sensor_msgs.PointCloud2()
    
        # Convert Vector3dVector points to a NumPy array
        points_np = np.asarray(self.pc.points)

        ros_dtype = sensor_msgs.PointField.FLOAT32
        dtype = np.float32
        itemsize = np.dtype(dtype).itemsize # A 32-bit float takes 4 bytes.

        data = points_np.astype(dtype).tobytes()
        ...
        header = std_msgs.Header(frame_id=pc_id)

        return sensor_msgs.PointCloud2(
            header=header,
            height=1, 
            width=points_np.shape[0],
            is_dense=False,
            is_bigendian=False,
            fields=fields,
            point_step=(itemsize * 3), # Every point consists of three float32s.
            row_step=(itemsize * 3 * points_np.shape[0]),
            data=data,
        )

    def deserialize(message) -> PointCloud:
        pc_data = read_points(message, field_names=("x", "y", "z"), skip_nans=True)

        # Convert the data to a NumPy array
        pc_np_array = np.array(list(pc_data), dtype=np.float32)

        # Create an Open3D point cloud from the NumPy array
        o3d_pc = o3d.geometry.PointCloud()
        o3d_pc.points = o3d.utility.Vector3dVector(pc_np_array)

        pc = PointCloud(point_cloud=o3d_pc)
        pc.round()

        return pc

    def round(self, decimals = PRECISION) -> None:
        pts = np.asarray(self.pc.points)
        np.around(pts, decimals=PRECISION, out=pts)


    def __repr__(self):
        return f"{str(self.pc)}\n{np.asarray(self.pc.points)}"
