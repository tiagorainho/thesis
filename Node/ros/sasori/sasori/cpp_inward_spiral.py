...
class InwardSpiralSolver(CPPSolver):
    ...
    def plan(self,
        ...
        logger.error(f"total: {len(traversable_pc.pc.points)}, goal: {len(goal_pc.pc.points)}")
        ...
        def get_neighbor_inward(current_position, step_size, closed_points, angle_offset=0):
            ...
            for point in points:
                ...
                covered_points_pc = PointCloud(points=covered_points)
                ...
                for real_neighbor in real_neighbors:
                    ...
                    closest_p = covered_points_pc.closest_neighbor(point=rounded_point, max_radius=2)
                    if closest_p is not None and dist(closest_p, rounded_point) < 0.5:
                        continue
        ...
        # log statistics
        len_covered = max(1, len(covered_points) - len(covered_points.intersection(path)))

        len_pc = max(1, len(goal_pc.pc.points))
        ...
        logger.error(f"pc points: {len(path)}")
