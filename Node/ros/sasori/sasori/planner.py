...
class Planner:
    ...
    def update_plan(self, traversable_pc: PointCloud, goal_pc: PointCloud, position: np.ndarray, previous_positions):
        ...
        # logger.error(f"covered_points: {covered_points}")
        ...
        new_plan_generator = self.solver.plan(
            traversable_pc=traversable_down_sampled_pc,
            goal_pc=goal_pc,
            start_point=position
        )

        new_plan = [new_coord for new_coord in new_plan_generator]
